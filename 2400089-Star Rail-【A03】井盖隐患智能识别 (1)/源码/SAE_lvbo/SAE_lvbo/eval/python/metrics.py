...
class F1Measure(object):
    ...
    def step(self, pred: np.ndarray, gt: np.ndarray):
        pred, gt = _prepare_data(pred, gt)
        if len(np.unique(gt)) < 2 or len(np.unique(pred)) < 2:  # Skip this batch if it cannot be used to compute F1
            return
        f1 = self.cal_f1(pred=pred, gt=gt)
        self.f1_scores.append(f1)

    def cal_f1(self, pred: np.ndarray, gt: np.ndarray) -> float:
        pred = (pred > self.threshold).astype(int)  # Convert predictions to class labels
        f1 = f1_score(y_true=gt.flatten(), y_pred=pred.flatten())
        return f1

    def get_results(self) -> dict:
        mean_f1 = np.mean(np.array(self.f1_scores)) if self.f1_scores else float('nan')
        return dict(f1=mean_f1)

class AUCMeasure(object):
    def __init__(self, threshold=0.5):
        self.auc_scores = []
        self.threshold = threshold
    ...
    def step(self, pred: np.ndarray, gt: np.ndarray):
        pred, gt = _prepare_data(pred, gt)
        if len(np.unique(gt)) < 2 or len(np.unique(pred)) < 2:  # Skip this batch if it cannot be used to compute AUC
            return
        auc = self.cal_auc(pred=pred, gt=gt)
        self.auc_scores.append(auc)

    def cal_auc(self, pred: np.ndarray, gt: np.ndarray) -> float:
        pred = (pred > self.threshold).astype(int)  # Convert predictions to class labels
        auc = roc_auc_score(y_true=gt.flatten(), y_score=pred.flatten())
        return auc

    def get_results(self) -> dict:
        mean_auc = np.mean(np.array(self.auc_scores)) if self.auc_scores else float('nan')
        return dict(auc=mean_auc)
...
class BER(object):
    ...
    def step(self, pred: np.ndarray, gt: np.ndarray):
        ...
        pos_err, neg_err = self.cal_ber(pred, gt)
        self.pos_errs.append(pos_err)
        self.neg_errs.append(neg_err)

    def cal_ber(self, pred: np.ndarray, gt: np.ndarray) -> tuple:
        # print(np.max(pred))
        pos_errs = []
        neg_errs = []
        eps = 1e-10  # A small constant

        # Assume that the second dimension represents the labels
        for i in range(pred.shape[1]):
            adaptive_threshold = _get_adaptive_threshold(pred[:, i], max_value=1)
            pred_i = (pred[:, i] >= adaptive_threshold).astype(int)
            gt_i = (gt[:, i] > 0.5).astype(int)

            if len(np.unique(pred_i)) == 1 or len(np.unique(gt_i)) == 1:
                # If pred_i or gt_i only contains one unique value
                if np.unique(pred_i)[0] == 1 and np.unique(gt_i)[0] == 1:
                    # If the unique value is 1 (positive class)
                    tn, fp, fn, tp = 0, 0, 0, len(pred_i)
                else:
                    # If the unique value is 0 (negative class)
                    tn, fp, fn, tp = len(pred_i), 0, 0, 0
            else:
                tn, fp, fn, tp = confusion_matrix(gt_i, pred_i).ravel()

            pos_err = (1 - tp / (tp + fn + eps)) * 100
            neg_err = (1 - tn / (tn + fp + eps)) * 100

            pos_errs.append(pos_err)
            neg_errs.append(neg_err)

        # Average over all labels
        pos_err = np.mean(np.array(pos_errs))
        neg_err = np.mean(np.array(neg_errs))

        return pos_err, neg_err

    def get_results(self) -> dict:
        pos_err = np.mean(np.array(self.pos_errs))
        neg_err = np.mean(np.array(self.neg_errs))
        ber = (pos_err + neg_err) / 2

        return dict(pos_err=pos_err, neg_err=neg_err, ber=ber)
