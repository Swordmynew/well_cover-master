...
class fusion(nn.Module): #gConv
    ...
    def forward(self, x, xa):
        xa = F.interpolate(xa, size=[88,88], mode='bilinear')
        x = F.interpolate(x, size=[88,88], mode='bilinear')
        xx = torch.cat((x, xa), 1)
        xx1 = self.g1(xx)       
        xx2 = self.g2(xx)
        xx3 = self.g3(xx)
...
class fusion3(nn.Module): #gConv with different channels
    ...
    def forward(self, xx):
        xx = F.interpolate(xx, size=[88,88], mode='bilinear')
        xx1 = self.g1(xx)
        xx2 = self.g2(xx)
        xx3 = self.g3(xx)
        xx4 = self.g4(xx)
...
class Decoder(nn.Module):
    ...
    def get_edge_cv(self, input_tensor):
        input_tensor = F.interpolate(input_tensor, size=[352,352], mode='bilinear')
        kernel = torch.tensor([[-1, -1, -1], [-1, 8, -1], [-1, -1, -1]], dtype=torch.float32).cuda()
        ...
        for i in range(batch_size):
            for j in range(channels):
                normalized_tensor = (input_tensor[i,j] - input_tensor[i,j].min()) / (input_tensor[i,j].max() - input_tensor[i,j].min())
                thresholded_tensor = (normalized_tensor > 0.5).float()
                edge_detection = F.conv2d(thresholded_tensor.unsqueeze(0).unsqueeze(0), kernel.unsqueeze(0).unsqueeze(0), padding=1)
                edge_detection = torch.abs(edge_detection)
    
                # Convert tensor to numpy array for cv2 operations
                edge_detection_np = edge_detection.squeeze(0).cpu().numpy()
                # Convert back to tensor
                edge_detection = torch.from_numpy(edge_detection_np).to('cuda')
    
                input_tensor[i,j] = edge_detection.squeeze(0)
    ...
    def lvbo1(self, D, original_img, shape):
        ...
        original_img = F.interpolate(original_img, size=shape, mode='bilinear')
        ...
        for i in range(D.size(0)):
            ...
            filtered_images.append(0.6 * torch.from_numpy(img_back).cuda() + 0.3*D_resized.detach())
    ...
    def lvbo1_edge(self, D, original_img, shape, k_size):
        D = F.interpolate(D, size=shape, mode='bilinear')
        original_img = F.interpolate(original_img, size=shape, mode='bilinear')
        original_img = original_img * 255 
        filtered_images = []
        ...
        D = torch.where(D < 0.1, 0.0, D.to(torch.double))
        D = torch.where(D >= 0.1, 1.0, D.to(torch.double)).to(torch.float)
    
        D = F.max_pool2d(D, kernel_size=k_size, stride=2)
        D = F.interpolate(D, size=shape, mode='bilinear')
        original_img = torch.mean(original_img, dim=1, keepdim=True)
        for i in range(D.size(0)):
            D_sum = D[i]
            ...
            if high_val_points.shape[1] == 0:
                img_back = torch.zeros_like(original_img[i])
                filtered_images.append(img_back)
                #print("11111high_val_points.shape[1] == 0",torch.max(D_resized))
                continue
            top = torch.min(high_val_points[1])
            bottom = torch.max(high_val_points[1])
            left = torch.min(high_val_points[2])
            ...
            window = (original_img*D_resized)[i, :, max(0, top):min(shape[0], bottom), max(0, left):min(shape[1], right)] 
            window = window.float()
            if window.numel() <= 0:
                img_back = torch.zeros_like(original_img[i])
                filtered_images.append(img_back)
            ...
            # Binarize the window's frequency domain with a threshold of 0.5
            fshift_window = (np.abs(fshift_window) >= 0.5).astype(np.float32)
            
            # Create a mask to filter out the low frequency components
            rows, cols = fshift_window.shape
            crow, ccol = rows // 2, cols // 2
            y, x = np.ogrid[-crow:rows-crow, -ccol:cols-ccol]
            distance = np.sqrt(x*x + y*y)
            sorted_distance = np.sort(distance[fshift_window == 1])
            if len(sorted_distance) == 0:
                filtered_images.append(torch.zeros_like(original_img[i]))
                continue
            threshold_distance = sorted_distance[int(len(sorted_distance) * 0.5)]  
            mask = distance >= threshold_distance
            
            # Apply the mask to the binarized frequency domain of the window
            fshift_window_filtered = fshift_window * mask
            ...
            img_back = np.abs(img_back)
            img_back = (img_back - np.min(img_back)) / (np.max(img_back) - np.min(img_back) + 1e-8)
            #print(torch.from_numpy(img_back).shape)
            filtered_images.append(torch.from_numpy(img_back).cuda())
        if len(filtered_images) == 0:
            ...
            filtered_images = torch.stack(filtered_images, dim=0).float().cuda()
            return filtered_images
    def lvbo_edge(self, D, original_img, shape, k_size):
        b, g, r = original_img[:,0:1,:,:],original_img[:,1:2,:,:],original_img[:,2:3,:,:]
        #print(b.shape)
        b_filtered = self.lvbo1_edge(D, b, shape, k_size)
        g_filtered = self.lvbo1_edge(D, g, shape, k_size)
        r_filtered = self.lvbo1_edge(D, r, shape, k_size)
        # Concatenate the filtered channels into a three-channel image
        filtered_image = torch.cat((b_filtered, g_filtered, r_filtered), dim=1)
        # Sum the channels into a one-channel image
        filtered_image = torch.stack((filtered_image,), dim=1).squeeze(1)
    ...
    def forward(self, E4, E3, E2, E1, shape, original_img):
        ...
        #Edge_f5 = self.get_edge_feature5(E5)
        #Edge5 = self.edge_out5(F.interpolate(Edge_f5,size=shape1,mode='bilinear'))
        Edge5 = self.get_edge_cv(P5)
        L5e = self.lvbo_edge(Edge5.detach(),original_img,shape,k_size = 7)
        L51 = (L51 + 0.1 * F.interpolate(L5e,size=L51.size()[2:],mode='bilinear'))
        ...
        P41 = P4
        #state2
        Edge4 = self.get_edge_cv(P4)
        #Edge_f4 = self.get_edge_feature4(D4,E3)
        #Edge4 = self.edge_out4(F.interpolate(Edge_f4,size=shape1,mode='bilinear'))
        L4e = self.lvbo_edge(Edge4.detach(),original_img,shape,k_size = 5)
        L41 = self.lvbo(P41,original_img,shape)
        L41 = (L41 + 0.1 * F.interpolate(L4e,size=L41.size()[2:],mode='bilinear'))
        ...
        #state3
        Edge3 = self.get_edge_cv(P3)
        #Edge_f3 = self.get_edge_feature3(D3,E2)
        #Edge3 = self.edge_out3(F.interpolate(Edge_f3,size=shape1,mode='bilinear'))
        L3e = self.lvbo_edge(Edge3.detach(),original_img,shape,k_size = 3)
        L31 = self.lvbo(P3,original_img,shape)
        L31 = (L31 + 0.1 * F.interpolate(L3e,size=L31.size()[2:],mode='bilinear'))
        ...
        #state4
        Edge2 = self.get_edge_cv(P2)
        #Edge_f2 = self.get_edge_feature2(D2,E1)
        #Edge2 = self.edge_out2(F.interpolate(Edge_f2,size=shape1,mode='bilinear')) 
        L2e = self.lvbo_edge(Edge2.detach(),original_img,shape,k_size = 2)
        E1 = F.interpolate(E1,size=(88,88),mode='bilinear')
        L21 = self.lvbo(P2,original_img,shape)
        L21 = (L21 + 0.1 * F.interpolate(L2e,size=L21.size()[2:],mode='bilinear'))
        ...
        P1 = F.interpolate(P1, size=shape1, mode='bilinear')
        P2 = F.interpolate(P2, size=shape1, mode='bilinear')
        P3 = F.interpolate(P3, size=shape1, mode='bilinear')
        P4 = F.interpolate(P4, size=shape1, mode='bilinear')
        P5 = F.interpolate(P5, size=shape1, mode='bilinear')
        ...
        L21 = F.interpolate(L21, size=shape1, mode='bilinear')
        L31 = F.interpolate(L31, size=shape1, mode='bilinear')
        L41 = F.interpolate(L41, size=shape1, mode='bilinear')
        L51 = F.interpolate(L51, size=shape1, mode='bilinear')
        ...
        Edge2 = F.interpolate(Edge2, size=shape1, mode='bilinear')
        Edge3 = F.interpolate(Edge3, size=shape1, mode='bilinear')
        Edge4 = F.interpolate(Edge4, size=shape1, mode='bilinear')
        Edge5 = F.interpolate(Edge5, size=shape1, mode='bilinear')
         
        return [P5, P4, P3, P2, P1], [F1, F2, F3, F4], [L21,L31,L41,L51], [Edge5,Edge4,Edge3,Edge2]
