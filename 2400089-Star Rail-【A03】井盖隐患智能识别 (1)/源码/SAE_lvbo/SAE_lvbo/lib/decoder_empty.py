...
class EEA_module(nn.Module):
    ...
    def forward(self, x, side_x, mask, edge, edge_f):
        ...
        mask_d = torch.ones_like(mask_d)
        edge = torch.ones_like(edge)
        edge_f = torch.ones_like(edge_f)
        side_x = torch.ones_like(side_x)
...
class Decoder(nn.Module):
    ...
    def forward(self, E5, E4, E3, E2, E1, shape, x_g):
        # For skip connection
        ...
        return [P5, P4, P3, P2, P1], [Pmax5, Pmax4, Pmax3, Pmax2], [Pmin5, Pmin4, Pmin3, Pmin2], [E5e, E4e, E3e, E2e], [P5e, P4e, P3e, P2e], [F1, F2, F3, F4], x_g
